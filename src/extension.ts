...
let playingProcess: ChildProcess | null = null;
...
// Sound configuration based on error count
...
const SUCCESS_SOUND = {
  soundFile: "shabash-beta.wav", // You can add your own success sound file
  description: "Success - Errors Fixed!",
};

// Get appropriate sound based on error count
const getSoundForErrorCount = (
  errorCount: number,
  extensionPath: string
): { path: string; description: string } => {
  for (const config of SOUND_CONFIG) {
    if (
      errorCount >= config.minErrors &&
...
const getSuccessSound = (
  ...
  const defaultSuccessPath = path.join(
    extensionPath,
    "media",
    "success-default.wav"
  );
...
const playSuccessSound = (soundPath: string, description: string) => {
  ...
  try {
    if (process.platform === "win32") {
      try {
        audioProcess = spawn(
          "cmd",
          ["/c", `start /min wmplayer.exe "${soundPath}"`],
          {
            windowsHide: true,
            shell: true,
          }
      ...
      audioProcess = spawn("afplay", [soundPath]);
    } else {
      audioProcess = spawn("aplay", [soundPath]);
    }
...
const playSuccessSoundWeb = (soundPath: string, description: string) => {
  if (!audioPanel) {
...
const playNativeSound = (soundPath: string): ChildProcess | null => {
  ...
  audioPanel.webview.html = `
    ...
    <body>
        ...
        <div class="error-info" id="errorInfo">
            <div class="error-count" id="errorCount">${errorCount} Error${
    errorCount !== 1 ? "s" : ""
  }</div>
...
const playWebAudio = (soundPath: string) => {
  const context = getCurrentContext();
  if (!context) return;

  const soundInfo = getSoundForErrorCount(
    currentErrorCount,
...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (missingSounds.length > 0) {
    vscode.window.showWarningMessage(
      `Missing sound files: ${missingSounds.join(
        ", "
      )}. Please add them to the media folder.`
    );
  }
  ...
  // Register commands
  const toggleAudioCommand = vscode.commands.registerCommand(
    "errorSound.toggleAudioMethod",
    () => {
  ...
  const showAudioPanelCommand = vscode.commands.registerCommand(
    "errorSound.showAudioPanel",
    () => {
      const soundInfo = getSoundForErrorCount(
        currentErrorCount || 1,
        context.extensionPath
  ...
  const testAudioCommand = vscode.commands.registerCommand(
    "errorSound.testAudio",
    () => {
      vscode.window.showInformationMessage(
        "Testing audio with current error count..."
      );
      ...
      setTimeout(() => {
        ...
        vscode.window.showInformationMessage(
          `Audio test completed (${soundInfo.description})`
        );
      }, 3000);
    }
  );
  ...
  const testSuccessCommand = vscode.commands.registerCommand(
    "errorSound.testSuccess",
    () => {
